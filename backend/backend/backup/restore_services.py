...
class RestoreService:
    ...
    def run(self):
        ...
        if self.restore.status != Restore.Status.QUEUE:
            return {
    ...
    def _validate_dump_file(self, dump_path):
        ...
        # Detect pg_restore version and check compatibility with the archive.
        pg_restore_version = detect_pg_restore_version(pg_restore_binary)
        pg_restore_major = extract_major_version(pg_restore_version)

        # Check pg_restore and archive client compatibility before validating.
