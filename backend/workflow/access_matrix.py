...
def _permission_scope_qs(workflow, subject_type, subject_value):
    subject = _subject_kwargs(subject_type, subject_value)
    return WorkflowPermission.objects.filter(workflow=workflow, **subject)
...
def _set_field_access(
    ...
    *,
    ...
    subject = _subject_kwargs(subject_type, subject_value)
    qs = FieldAccess.objects.filter(field=field, step=step, **subject)
...
def _set_group_access(
    ...
    *,
    ...
    subject = _subject_kwargs(subject_type, subject_value)
    qs = RepeatableGroupAccess.objects.filter(group=group, step=step, **subject)
