...
class EditModeAuthorizationTests(TestCase):
    ...
    def test_device_delete_endpoint_allows_can_delete_without_can_edit(self):
        """Delete endpoint works with can_delete=True even when can_edit=False."""
        ...
        # Delete is allowed independently of edit permission
        rga = RepeatableGroupAccess.objects.get(
            group=self.device_group,
            step=self.step,
            user=self.user,
        ...
        url = reverse(
            ...
            args=[
                instance.pk,
                self.device_group.code,
                instance_device.pk,
            ],
        )

        response = self.client.post(f"{url}?edit=1")
        ...
        # Persistent Device itself must remain unchanged
        device.refresh_from_db()
        self.assertTrue(
            Device.objects.filter(pk=device.pk).exists()
        )

    def test_device_delete_endpoint_rejected_without_edit_mode(self):
        """Delete endpoint rejects deletion when edit_mode=False."""
        self._create_device_group()
        instance = self._create_instance()
    ...
    def test_device_delete_endpoint_rejected_when_submitted(self):
        """Delete endpoint rejects deletion after the step is submitted."""
        self._create_device_group()
        instance = self._create_instance()
