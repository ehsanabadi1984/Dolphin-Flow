...
class FormulaServiceTests(SimpleTestCase):
    ...
    def test_round_floor_and_ceil_functions(self):
        ...
        self.assertEqual(
            FormulaService.evaluate_tokens(tokens=round_tokens, field_resolver=resolver),
            Decimal("10.13"),
        )
        self.assertEqual(
            FormulaService.evaluate_tokens(tokens=floor_tokens, field_resolver=resolver),
            Decimal("10"),
        )
        self.assertEqual(
            FormulaService.evaluate_tokens(tokens=ceil_tokens, field_resolver=resolver),
            Decimal("11"),
        )
    ...
    def test_cross_section_top_level_field_reference_is_allowed(self):
        formula_field = SimpleNamespace(
            pk=2,
            field_type=FormulaService.FIELD_TYPE,
            repeatable_group_id=None,
            label="قیمت نهایی",
        )
        number_in_other_section = SimpleNamespace(
            pk=1,
    ...
    def test_row_formula_cannot_reference_other_group(self):
        formula_field = SimpleNamespace(
            pk=3,
            field_type=FormulaService.FIELD_TYPE,
            repeatable_group_id=100,
            label="جمع ردیف",
        )
        number_in_other_group = SimpleNamespace(
            pk=1,
            field_type=FormField.FieldType.NUMBER,
            repeatable_group_id=200,
            label="مبلغ",
        )
